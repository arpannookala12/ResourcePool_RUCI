...
# Load the dataset
file_path = '/mnt/data/smsmresults.csv'
data = pd.read_csv(file_path)

# Step 1: Data Cleaning and Preparation
# Ensure 'TWDAYS' is consistent and treated as a categorical variable
data['TWDAYS'] = data['TWDAYS'].str.strip().str.lower()
data['TWDAYS'] = data['TWDAYS'].replace({
    'yes, for 1-2 days': 'Yes, for 1-2 days', 
    'yes, for 3-4 days': 'Yes, for 3-4 days', 
    'yes, for 5 or more days': 'Yes, for 5 or more days',
    'no': 'No'
})
...
data['TWDAYS'] = data['TWDAYS'].astype('category')
...
age_twday_proportion = data.groupby(['AGE', 'TWDAYS']).size().groupby(level=0).apply(lambda x: x / x.sum()).unstack(fill_value=0)

# Summarize by gender
gender_twday_summary = data.groupby(['EGENID_BIRTH', 'TWDAYS']).size().unstack(fill_value=0)

# Calculate proportions by gender
gender_twday_proportion = data.groupby(['EGENID_BIRTH', 'TWDAYS']).size().groupby(level=0).apply(lambda x: x / x.sum()).unstack(fill_value=0)
...
education_twday_proportion = data.groupby(['EEDUC', 'TWDAYS']).size().groupby(level=0).apply(lambda x: x / x.sum()).unstack(fill_value=0)

# Step 3: Visualization

# Visualization for telework days by age group
age_twday_proportion.plot(kind='bar', stacked=True, figsize=(12, 8))
...
gender_twday_proportion.plot(kind='bar', stacked=True, figsize=(10, 6))
plt.ylabel('Proportion')
...
education_twday_proportion.plot(kind='bar', stacked=True, figsize=(14, 10))
plt.ylabel('Proportion')
...
tools.display_dataframe_to_user(name="Telework Days by Age Group", dataframe=age_twday_summary)
tools.display_dataframe_to_user(name="Telework Days by Gender", dataframe=gender_twday_summary)
tools.display_dataframe_to_user(name="Telework Days by Educational Attainment", dataframe=education_twday_summary)

tools.display_dataframe_to_user(name="Telework Proportion by Age Group", dataframe=age_twday_proportion)
tools.display_dataframe_to_user(name="Telework Proportion by Gender", dataframe=gender_twday_proportion)
tools.display_dataframe_to_user(name="Telework Proportion by Educational Attainment", dataframe=education_twday_proportion)
```